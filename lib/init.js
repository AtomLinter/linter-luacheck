...
const makeParameters = function makeParameters(globals, ignore, std, file) {
  let parameters = [
    '-', '--no-color', '--codes',
  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        const std = atom.config.get('linter-luacheck.standards');
        ...
        const parameters = makeParameters(globals, ignore, std, file);
